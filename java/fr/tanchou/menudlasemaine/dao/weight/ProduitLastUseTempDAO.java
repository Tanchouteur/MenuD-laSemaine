...
public class ProduitLastUseTempDAO {

    public static void updateLastUseDate(String nomProduit) {
        String sql = """
            UPDATE ProduitLastUseTemp 
    ...
    public static void upsertProduitLastUse(String nomProduit, String typeProduit) {
        String sql = """
                INSERT INTO ProduitLastUseTemp (nom_produit, type_produit, date_last_use) 
                VALUES (?, ?, CURRENT_DATE) 
    ...
    public static Optional<Date> getLastUseDate(String nomProduit, TypeProduit typeProduit) {
        String sql = "SELECT date_last_use FROM ProduitLastUseTemp WHERE nom_produit = ? AND type_produit = ?";
        try (Connection conn = DatabaseConnection.getDataSource().getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, nomProduit);
    ...
    public static void deleteProduitLastUse(String nomProduit, String typeProduit) {
        String sql = "DELETE FROM ProduitLastUseTemp WHERE nom_produit = ? AND type_produit = ?";
        try (Connection conn = DatabaseConnection.getDataSource().getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, nomProduit);
            pstmt.setString(2, typeProduit);
        ...
        switch (typeProduit) {
            case VIANDE:
                sql = "SELECT pl.nom_produit, v.poids, pl.date_last_use " +
                        "FROM ProduitLastUseTemp pl JOIN Viande v ON pl.nom_produit = v.nom_viande " +
                        "WHERE pl.type_produit = ?";
                break;
            case FECULENT:
                sql = "SELECT pl.nom_produit, f.poids, pl.date_last_use " +
                        "FROM ProduitLastUseTemp pl JOIN Feculent f ON pl.nom_produit = f.nom_feculent " +
                        "WHERE pl.type_produit = ?";
                break;
            case LEGUME:
                sql = "SELECT pl.nom_produit, l.poids, pl.date_last_use " +
                        "FROM ProduitLastUseTemp pl JOIN Legume l ON pl.nom_produit = l.nom_legume " +
                        "WHERE pl.type_produit = ?";
                break;
            case ENTREE:
                sql = "SELECT pl.nom_produit, e.poids, pl.date_last_use " +
                        "FROM ProduitLastUseTemp pl JOIN Entree e ON pl.nom_produit = e.nom_entree " +
                        "WHERE pl.type_produit = ?";
                break;
            case PLAT_COMPLET:
                sql = "SELECT pl.nom_produit, pc.poids, pl.date_last_use " +
                        "FROM ProduitLastUseTemp pl JOIN PlatComplet pc ON pl.nom_produit = pc.nom_plat " +
                        "WHERE pl.type_produit = ?";
                break;
        }
